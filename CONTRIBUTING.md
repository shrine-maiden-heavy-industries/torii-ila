...
Hey! Thanks for your interest in contributing to Torii ILA!

A few important things to note up front:

* Contributions to the Torii ILA source code are licensed under the [BSD-3-Clause], the full text of which can be found in the [`LICENSE`] file in the root of the [git repository].
* Contributions to the Torii ILA documentation are licensed under the [CC-BY-SA 4.0], the full text of which can be found in the [`LICENSE.docs`] file in the root of the [git repository].
* When contributing to Torii ILA and interacting with people in/around the project be sure to keep in mind the [Code Of Conduct].
...
We are trying to make Torii ILA as widely useable and capable as possible, as such we love hearing about things that would help us achieve that goal.
...
When working with the Torii ILA code base, we require the following tools:
...
1. Within the local repository, add this repo as a remote: `git remote add upstream https://github.com/shrine-maiden-heavy-industries/torii-ila`
...
To run the Torii ILA test suite, you can simply invoke `nox -s test`, this will do everything needed to set up Torii ILA and then invoke it's test suite on the current state of the code base as it is locally.
...
* If the commit touches part of the core Torii ILA package, try to use the full dotted path to the module. (e.g. `torii_ila.usb`, `torii_ila.ila`, etc. )
...
```text
docs: Fixed a pile of typos in the language guide
```

```text
torii_ila.uart: Added re-prime command for ILA trigger

The UART ILA used to be mono-directional, only dumping one sample payload once the trigger is hit and then it was inert.

This commit adds a control channel to allow us to send commands to the ILA engine to do things such
as re-prime the ILA trigger, or set it to auto-re-prime so it is always sending back data.
```
...
Next, the `$function` section can be thought of something like a filesystem path segment that describes where this branch is focused, for instance, `backhaul/usb` if it's related to the USB backhaul components. If the changes are more wide-reaching than a specific sub-section, then this part can be omitted.

Finally, the `$name` section should be a short, but descriptive name for what this branch is doing, it doesn't need to be super detailed, that's what commit messages are for, but something that gives a general idea, like `linux-epipe-fix`.
...
And with that, you're done! You've contributed to Torii ILA!

## Documentation

The documentation for Torii ILA uses [sphinx] with a few plugins, most notably [MyST], which allows us to use Markdown with some extensions to do the documentation rather than reStructuredText.
...
Torii ILA explicitly does not allow any contributions that were in either whole or in part made using a Generative AI system or agent, such as large language models (LLMs), chatbots, or image generation systems. This ban includes, but is not limited to Copilot, BARD/Gemini, ChatGPT, Claude, llama, DeepSeek, Stable Diffusion, DALL-E, Midjourney, or Devin AI.

This policy covers ***ALL*** parts of this project, including but not limited to code, tests, documentation, metadata, artworks, issues, comments, suggestions, feature requests, discussions, pull requests, and any other possible contributions that can be made to Torii ILA.

If you are found to be using such tools when contributing to Torii ILA, all of you contributions will be removed if viable, outstanding contributions closed without recourse or review, all future collaboration efforts denied, and you will be banned from contributing to Torii ILA and related projects.
...
[issue tracker]: https://github.com/shrine-maiden-heavy-industries/torii-ila/issues
[report an issue]: https://github.com/shrine-maiden-heavy-industries/torii-ila/issues
...
[fork]: https://github.com/shrine-maiden-heavy-industries/torii-ila/fork
...
[`docs`]: https://github.com/shrine-maiden-heavy-industries/torii-ila/blob/main/docs
[testing]: #testing
[new pull request]: https://github.com/shrine-maiden-heavy-industries/torii-ila/compare
