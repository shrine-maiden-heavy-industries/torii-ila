...
class _bits_base(Sequence):
	...
	_bytestype: type[bytes] | type[bytearray]

	@classmethod
	def from_int(cls: type[Self], value: int, length: int | None = None) -> Self:
		'''
	...
	@classmethod
	def from_iter(cls: type[Self], iterator: Iterable[int]) -> Self:
		'''
	...
	@classmethod
	def from_bytes(cls, value: bytes, length: int | None = None) -> Self:
		'''
	...
	def __new__(cls, value = 0, length: int | None = None) -> Self:
		'''
		...
		if isinstance(value, _bits_base):
			if length is not None:
				raise ValueError(
					f'invalid input for {cls.__name__}(): when converting from bits length must not be provided'
				)
		...
		if isinstance(value, str):
			if length is not None:
				raise ValueError(
					f'invalid input for {cls.__name__}(): when converting from str length must not be provided'
				)
		...
		if isinstance(value, Iterable):
			if length is not None:
				raise ValueError(
					f'invalid input for {cls.__name__}(): when converting from an iterable length must not be provided'
				)
	...
	def __getitem__(self: Self, key) -> Self | int:
	...
	def _bitop(self: Self, other: object, op) -> Self:
	...
	def __xor__(self: Self, other: object) -> Self:
	...
	def byte_reversed(self: Self) -> Self:
		'''
	...
	def find(self: Self, needle: object, start: int = 0, end: int | None = None) -> int:
		'''
...
class bitarray(_bits_base, MutableSequence):
	...
	def _fix_padding(self: Self) -> None:
		if self._len % 8 != 0:
			self._bytes[-1] &= ~(-1 << (self._len % 8))

	def _resize(self: Self, length: int) -> None:
	...
	def __setitem__(self: Self, key: object, value: object) -> None:
		if isinstance(key, slice):
			start, stop, step = key.indices(self._len)
	...
	def byte_reverse(self: Self) -> None:
		'''
	...
	def extend(self: Self, values: object) -> None:
		if isinstance(values, (str, _bits_base)):
			self[self._len:] = values
		else:
			super().extend(values)

	def __imul__(self: Self, other: object) -> Self:
	...
	def _ibitop(self: Self, other: object, op) -> Self:
		if isinstance(other, int):
			other = bits(other, self._len)
		elif not isinstance(other, _bits_base):
			other = bits(other)
	...
	def __iand__(self: Self, other: object) -> Self:
		return self._ibitop(other, operator.__and__)

	def __ior__(self: Self, other: object) -> Self:
		return self._ibitop(other, operator.__or__)

	def __ixor__(self: Self, other: object) -> Self:
		return self._ibitop(other, operator.__xor__)

	def setall(self: Self, value: SupportsIndex) -> None:
